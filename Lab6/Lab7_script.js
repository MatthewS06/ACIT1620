...
let button = document.getElementById("colorButtons")
let currentcolor = document.getElementById("colorBox")
let addcolor = document.getElementById("addColorForm")
...
button.addEventListener("click", function() {
  let elements = document.getElementsByClassName("color-box")
  let choice = event.target;
  let color = choice.style;
  for (let i = 0; i < elements.length; i++) {
    elements[i].style.backgroundColor = color.backgroundColor;
  }
  currentcolor.innerHTML = color.backgroundColor;
});
